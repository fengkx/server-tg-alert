...
const https = __toModule(require("https"));
const zlib = __toModule(require("zlib"));
...
const STATUS_CODES = http.default.STATUS_CODES;
class Response {
  constructor() {
...
async function run() {
  ...
  if (memUsedPrecent >= memUsedBound) {
    message = `**ALERT**: ${NAME} Memory used ${data.mem.used / GB}GB, ${Math.floor(memUsedPrecent * 1e4) / 100}%`;
  }
  ...
  if (message) {
    index_default(`https://api.telegram.org/${TELEGRAM_BOT_TOKEN}/sendMessage`, {
      method: "POST",
      body: JSON.stringify({
        chat_id: SEND_TO,
        text: message,
